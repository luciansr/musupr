module MusuprApp {
    ...
    angular.module('MusuprApp').controller('VagaController', ['VagasService', '$routeParams', 'LogService', '$location',
        function (VagasService: VagasService, $routeParams, LogService: LogService, $location: ng.ILocationService) {
            var self = this;
            ...
            self.AvaliaVaga = function (like) {
                VagasService.AvaliaVaga(like, self.id).then(
                    function (success) {
                        if (success) {
            ...
            init();
            function init() {
                VagasService.GetVagaByID($routeParams.id).then(
                    function (vaga) {
                        self.vaga = vaga;
                    },
                    function (error) {
