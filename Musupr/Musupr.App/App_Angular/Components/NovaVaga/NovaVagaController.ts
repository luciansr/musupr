module MusuprApp {
    angular.module('MusuprApp').controller('NovaVagaController', ['VagasService', '$scope', 'AccountService', '$routeParams', 'LogService', '$location', 'vcRecaptchaService',
        function (VagasService: VagasService, $scope: ng.IScope, AccountService: AccountService, $routeParams, LogService: LogService, $location, vcRecaptchaService) {
            var self = this;
            var agora = new Date();
            var depois = new Date();
            ...
            self.SalvarAnuncio = function () {
                ...
                VagasService.SalvarAnuncio(self.vaga, reCaptchaResponse).then(
                    function (data) {
            ...
            function init() {
                ...
                if ($routeParams.id == undefined || $routeParams == null) {
                    ...
                    VagasService.GetVagaParaEditarByID($routeParams.id).then(
                        function (vaga) {
                            self.vaga = vaga;
                ...
                VagasService.GetTiposDeVagas().then(
                    function (data) {
                        self.tiposDeVaga = [];
                        for (var i in data) {
