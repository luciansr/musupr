module MusuprApp {
    angular.module('MusuprApp').controller('AccountController', ['$scope', 'AccountService', '$location', 'LogService', '$q', 'vcRecaptchaService',
        function ($scope: any, AccountService: AccountService, $location: ng.ILocationService, LogService: LogService, $q, vcRecaptchaService) {
            var self = this;
            ...
            self.RegistrarUsuario = function () {
                ...
                AccountService.RegistrarUsuario(self.newUser.Nome, self.newUser.userID, self.newUser.email, self.newUser.password, reCaptchaResponse).then(
                    function (data) {
                        LogService.Success('Bem vindo(a), ' + self.newUser.Nome + '!');

                        AccountService.login(self.newUser.userID, self.newUser.password).then(
                            function (success) {
                                if (success) {
                                    $location.path('/');
                                }
            ...
            self.login = function (submit) {
                if (submit === undefined || submit) {
                    AccountService.login(self.userID, self.password).then(
                        function (success) {
                            if (success) {
                                $location.path('/');
                            }
